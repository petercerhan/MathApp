//
//  ConceptTableViewCell+utilities.swift
//  MathAppTests
//
...
extension ConceptTableViewCell {
    ...
    static func assertCellAtRow(_ row: Int, inTable table: UITableView, showsName name: String, file: StaticString = #file, line: UInt = #line) {
        let assertion = {
            guard let cell = table.cellForRow(at: IndexPath(row: row, section: 0)) as? ConceptTableViewCell else {
